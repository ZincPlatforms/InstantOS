// Headless P4 binutils oracle (/bin/login). Spawns /bin/hellocpp -- a program
...
// GNU-ld output), runs, and prints its marker. Reports GCC_CXX_* via serial.
...
static const char* const ARGV_HELLO[]={"hellocpp",0};
static const char* HELLO_MSG = "HELLO_FROM_GXX_ON_INSTANTOS\n";

void _start(void){
    serial("\n[GCC_CXX] start (run GNU as+ld hello)\n");
    ...
    i64 rc = spawn_wait("/bin/hellocpp", ARGV_HELLO, ENVP);
    serial("GCC_CXX_HELLO_RC="); put_dec(rc); serial("\n");

    int out_ok = file_matches("/tmp/hello.out", HELLO_MSG);
    serial(out_ok ? "GCC_CXX_HELLO_OUTPUT_OK\n" : "GCC_CXX_HELLO_OUTPUT_FAIL\n");

    if (rc == 0 && out_ok) serial("GCC_CXX_ALL_OK\n");
    serial("GCC_CXX_DONE\n");
    sc1(SYS_EXIT,0);
}
