...
// GNU-ld output), runs, and prints its marker. Reports BINUTILS_* via serial.
...
void _start(void){
    serial("\n[BINUTILS] start (run GNU as+ld hello)\n");
    ...
    serial("BINUTILS_HELLO_RC="); put_dec(rc); serial("\n");

    int out_ok = file_matches("/tmp/hello.out", HELLO_MSG);
    serial(out_ok ? "BINUTILS_HELLO_OUTPUT_OK\n" : "BINUTILS_HELLO_OUTPUT_FAIL\n");

    if (rc == 0 && out_ok) serial("BINUTILS_ALL_OK\n");
    serial("BINUTILS_DONE\n");
    sc1(SYS_EXIT,0);
}
